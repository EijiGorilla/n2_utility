...
// Caveat: we need to compile utility point + utility line to generate dropdonw data
export async function dropdownQuery() {
  var query = utilityPointLayer.createQuery();
  query.outFields = ['CP', 'Company', 'Type'];
  query.orderByFields = ['CP'];
  query.groupByFieldsForStatistics = ['CP', 'Company'];

  const pointQuery = utilityPointLayer.queryFeatures(query).then((response: any) => {
    var stats = response.features;
    const values = stats.map((result: any, index: number) => {
    ...
    const pair = values.filter(
      (val: any, index: any) =>
        values.findIndex(
          (item: any) =>
            item.type === val.type && item.company === val.company && item.cp === val.cp,
        ) === index,
    );
  ...
  // Utility Line
  var queryL = utilityLineLayer.createQuery();
  queryL.outFields = ['CP', 'Company', 'Type'];
  queryL.orderByFields = ['CP'];
  queryL.groupByFieldsForStatistics = ['CP', 'Company'];

  const lineQuery = utilityLineLayer.queryFeatures(queryL).then((response: any) => {
    var stats = response.features;
    const values = stats.map((result: any, index: number) => {
      const attributes = result.attributes;
      const cp = attributes.CP;
